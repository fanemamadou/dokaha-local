...
def vente_form(request):
    ...
    if request.method == 'POST':
        ...
        plateaux = int(request.POST.get('plateaux', 0))
        prix = int(request.POST.get('prix_plateau', 0))
        montant = plateaux * prix
        Vente.objects.create(date=timezone.now().date(), client=client, type_vente=type_v, plateaux=plateaux, prix_plateau=prix, montant_total=montant)
        return redirect('/vente/')
    return render(request, 'gestion/vente_form.html', {'type_vente': type_v, 'title': f'📝 Vente de {type_v.capitalize()}'})
